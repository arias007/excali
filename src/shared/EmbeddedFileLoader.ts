...
export class EmbeddedFilesLoader {
  ...
  private getMarkdownImageSourceCandidates(src: string): string[] {
    ...
    try {
      ...
      const decodedPath = decode(url.pathname).replace(/\\/g, "/");
      ...
      if (vaultBasePath && normalizedPath.startsWith(`${vaultBasePath}/`)) {
        add(normalizedPath.substring(vaultBasePath.length + 1));
      }
      add(normalizedPath);
      add(normalizedPath.substring(normalizedPath.lastIndexOf("/") + 1));
    } catch (_) {}

    return candidates;
  }

  private async inlineRenderedMarkdownImages(
    file: TFile,
    mdDIV: HTMLDivElement,
  ): Promise<void> {
    const images = Array.from(
      mdDIV.querySelectorAll<HTMLImageElement>("img"),
    );
    ...
    for (const img of images) {
      ...
      if (rawSources.some((src) => src.startsWith("data:"))) {
        continue;
      }

      const candidates = rawSources.flatMap((src) =>
        this.getMarkdownImageSourceCandidates(src),
      );
      for (const candidate of candidates) {
        const embeddedFile = new EmbeddedFile(this.plugin, file.path, candidate);
        if (
          !embeddedFile.file &&
        ...
        const image = await this._getObsidianImage(embeddedFile, 1);
        if (!image?.dataURL) {
          continue;
        }
  ...
  private async convertMarkdownToSVG(
    ...
    const internalEmbeds = Array.from(
      mdDIV.querySelectorAll<HTMLElement>("span.internal-embed"),
    );
    ...
    await this.inlineRenderedMarkdownImages(file, mdDIV);
