...
type CompactToolbarPenId = "black" | "red" | `custom-${number}`;

type CompactToolbarResolvedPenStyle = Pick<
  AppState,
...
type CompactToolbarPenStyle = Partial<CompactToolbarResolvedPenStyle>;
  ...
  implements HoverParent
{
  ...
  private compactToolbarSubmenuCloseTimer: number | null = null;
  private compactToolbarOpenSubmenu:
    | "shape"
    | "pen"
    | "zoom"
    | "more"
    | null = null;
  private compactToolbarSubmenusInitialized = false;
  private compactToolbarActivePen: CompactToolbarPenId = "black";
  private compactToolbarPenPresetApplied = false;
  ...
  private getCompactToolbarPenStyleFromAppState(
    appState: AppState,
  ): CompactToolbarResolvedPenStyle {
    return {
      currentStrokeOptions: appState.currentStrokeOptions ?? null,
  ...
  private getCompactToolbarPenColor(pen: CompactToolbarPenId): string {
    return String(
      this.compactToolbarPenStyles[pen]?.currentItemStrokeColor ??
        (pen === "red" ? "#d03030" : "#111111"),
    );
  }

  private getCompactToolbarPenLabel(pen: CompactToolbarPenId): string {
    if (pen === "black") {
      return "黑笔";
    }
    if (pen === "red") {
    ...
    const index = Math.max(1, this.compactToolbarPenOrder.indexOf(pen) + 1);
    return `笔 ${index}`;
  }
  ...
  private ensureCompactPenMenu(): HTMLElement {
    ...
    if (!menu) {
      menu = this.ownerDocument.createElement("div");
      menu.className = "excalidraw-compact-pen-menu";
      menu.setAttribute("role", "menu");
      menu.setAttribute("aria-label", "笔预设");
      this.contentEl.appendChild(menu);
      this.destroyers.push(() => menu?.remove());
    }

    const signature = `${this.compactToolbarPenOrder.join("|")}|${
      this.compactToolbarPenOrder.length >= this.compactToolbarMaxPens
    }`;
    ...
    this.compactToolbarPenOrder.forEach((pen) => {
      const button = this.ownerDocument.createElement("button");
      button.className = `excalidraw-compact-pen-option excalidraw-compact-pen-option-${pen}`;
      if (pen !== "black" && pen !== "red") {
        button.classList.add("excalidraw-compact-pen-option-custom");
      }
      button.dataset.pen = pen;
      button.type = "button";
      button.setAttribute("aria-label", this.getCompactToolbarPenLabel(pen));
      button.setAttribute(
        "title",
        `${this.getCompactToolbarPenLabel(pen)} - 长按删除`,
      );
      menu.appendChild(button);
    });

    const addButton = this.ownerDocument.createElement("button");
    const limitReached =
      this.compactToolbarPenOrder.length >= this.compactToolbarMaxPens;
    addButton.className = "excalidraw-compact-pen-add";
    addButton.type = "button";
    addButton.disabled = limitReached;
    addButton.setAttribute("aria-label", limitReached ? "笔位已满" : "添加笔");
    addButton.setAttribute("title", limitReached ? "最多 6 支笔" : "添加笔");
    addButton.toggleClass("excalidraw-compact-pen-add-disabled", limitReached);
    menu.appendChild(addButton);

    return menu;
  }

  private addCompactToolbarPenPreset() {
    if (this.compactToolbarPenOrder.length >= this.compactToolbarMaxPens) {
      return;
    }
    const api = this.excalidrawAPI;
    const currentAppState = api?.getAppState();
    const pen =
      `custom-${this.compactToolbarNextCustomPenIndex++}` as CompactToolbarPenId;
    this.compactToolbarPenStyles[pen] = currentAppState
      ? this.getCompactToolbarPenStyleFromAppState(currentAppState)
      : { ...this.compactToolbarPenStyles[this.compactToolbarActivePen] };
    this.compactToolbarPenOrder.push(pen);
    const menu = this.ensureCompactPenMenu();
    menu.dataset.signature = "";
    this.ensureCompactPenMenu();
    this.applyCompactToolbarPen(pen);
  }

  private removeCompactToolbarPenPreset(pen: CompactToolbarPenId) {
    if (this.compactToolbarPenOrder.length <= 1) {
      return;
    }
    const index = this.compactToolbarPenOrder.indexOf(pen);
    if (index < 0) {
      return;
    }

    const wasActive = this.compactToolbarActivePen === pen;
    const fallbackPen =
      this.compactToolbarPenOrder[index + 1] ??
      this.compactToolbarPenOrder[index - 1] ??
      this.compactToolbarPenOrder.find((candidate) => candidate !== pen);
    this.compactToolbarPenOrder = this.compactToolbarPenOrder.filter(
      (candidate) => candidate !== pen,
    );
    delete this.compactToolbarPenStyles[pen];

    const menu = this.ensureCompactPenMenu();
    menu.dataset.signature = "";
    this.ensureCompactPenMenu();

    if (wasActive && fallbackPen) {
      this.compactToolbarPenPresetApplied = false;
      this.compactToolbarActivePen = fallbackPen;
      this.applyCompactToolbarPen(fallbackPen);
      return;
    }

    if (!this.compactToolbarPenOrder.includes(this.compactToolbarActivePen)) {
      this.compactToolbarActivePen = this.compactToolbarPenOrder[0];
    }
  ...
  private syncCompactToolbarActivePenStyleFromAppState(appState?: AppState) {
    const currentAppState = appState ?? this.excalidrawAPI?.getAppState();
    if (
      !this.compactToolbarPenPresetApplied ||
      !currentAppState ||
      currentAppState.activeTool?.type !== "freedraw"
    ) {
      return;
    }
    this.compactToolbarPenStyles[this.compactToolbarActivePen] =
  ...
  private updateCompactToolbarPenUi(appState?: AppState) {
    ...
    this.contentEl.toggleClass(
      "excalidraw-compact-pen-menu-visible",
      isFreedrawActive,
    );
    if (isFreedrawActive) {
      this.syncCompactToolbarActivePenStyleFromAppState(currentAppState);
    }
    const activeColor = this.getCompactToolbarPenColor(
      this.compactToolbarActivePen,
    );
    this.contentEl.style.setProperty(
      "--excalidraw-compact-active-pen-color",
      activeColor,
    );
    this.contentEl.toggleClass(
      "excalidraw-compact-active-pen-black",
      this.compactToolbarActivePen === "black",
    );
    this.contentEl.toggleClass(
      "excalidraw-compact-active-pen-red",
      this.compactToolbarActivePen === "red",
    );
    this.compactToolbarPenOrder.forEach((pen) => {
      const button = this.contentEl.querySelector<HTMLElement>(
        `.excalidraw-compact-pen-option-${pen}`,
      );
      if (!button) {
        return;
      }
      button.style.setProperty(
        "--excalidraw-compact-pen-color",
        this.getCompactToolbarPenColor(pen),
      );
      button.toggleClass(
        "excalidraw-compact-pen-option-active",
  ...
  private applyCompactToolbarPen(pen: CompactToolbarPenId) {
    ...
    const currentAppState = api.getAppState();
    if (this.compactToolbarPenPresetApplied) {
      this.syncCompactToolbarActivePenStyleFromAppState(currentAppState);
    }
    this.compactToolbarActivePen = pen;
    const penStyle = {
      ...this.getCompactToolbarPenStyleFromAppState(currentAppState),
      ...this.compactToolbarPenStyles[pen],
    };
    this.compactToolbarPenStyles[pen] = penStyle;
    this.compactToolbarPenPresetApplied = true;
    api.updateScene({
      appState: penStyle,
      captureUpdate: CaptureUpdateAction.NEVER,
    });
    api.setActiveTool({ type: "freedraw" });
    this.updateCompactToolbarPenUi();
    this.updateCompactToolbarLayout();
    this.updateCompactSelectedElementActionsPosition();
  }
  ...
  private setupCompactToolbarSubmenus() {
    ...
    this.compactToolbarSubmenusInitialized = true;

    const clearPendingClose = () => {
      if (this.compactToolbarSubmenuCloseTimer) {
        window.clearTimeout(this.compactToolbarSubmenuCloseTimer);
        this.compactToolbarSubmenuCloseTimer = null;
      }
    };

    const setSubmenu = (
      submenu: "shape" | "pen" | "zoom" | "more" | null,
    ) => {
      clearPendingClose();
      this.compactToolbarOpenSubmenu = submenu;
      this.contentEl.toggleClass(
        "excalidraw-compact-submenu-shape",
        submenu === "shape",
      );
      this.contentEl.toggleClass(
        "excalidraw-compact-submenu-pen",
        submenu === "pen",
      );
      this.contentEl.toggleClass(
        "excalidraw-compact-submenu-zoom",
        submenu === "zoom",
      );
      this.contentEl.toggleClass(
        "excalidraw-compact-submenu-more",
        submenu === "more",
      );
    };

    const closestTool = (target: HTMLElement | null) =>
      target?.closest(".ToolIcon, .dropdown-menu-button, .zoom-actions");

    const toolHasTestId = (target: HTMLElement | null, testId: string) =>
      Boolean(closestTool(target)?.querySelector(`[data-testid="${testId}"]`));

    const isShapeSubmenuTarget = (target: HTMLElement | null) =>
      [
        "toolbar-line",
        "toolbar-rectangle",
        "toolbar-diamond",
        "toolbar-ellipse",
        "toolbar-arrow",
      ].some((testId) => toolHasTestId(target, testId));

    const isZoomSubmenuTarget = (target: HTMLElement | null) =>
      Boolean(target?.closest(".zoom-actions, .zoom-button"));

    const isPenSubmenuTarget = (target: HTMLElement | null) =>
      Boolean(
    ...
    const isMoreSubmenuTarget = (target: HTMLElement | null) =>
      Boolean(target?.closest(".App-toolbar__extra-tools-trigger, .ToolIcon__lock"));

    const toggleSubmenu = (submenu: "shape" | "pen" | "zoom" | "more") => {
      setSubmenu(this.compactToolbarOpenSubmenu === submenu ? null : submenu);
    };

    let pendingPenLongPress:
      | {
          didDelete: boolean;
          pen: CompactToolbarPenId;
          pointerId: number;
          startX: number;
          startY: number;
          target: HTMLElement;
    ...
    const clearPendingPenLongPress = () => {
      if (!pendingPenLongPress) {
        return;
      }
      window.clearTimeout(pendingPenLongPress.timer);
      pendingPenLongPress.target.removeClass(
        "excalidraw-compact-pen-option-long-pressing",
      );
      pendingPenLongPress = null;
    };

    const startPendingPenLongPress = (
      penOption: HTMLElement,
      event: PointerEvent,
      pen: CompactToolbarPenId,
    ) => {
      clearPendingPenLongPress();
      penOption.addClass("excalidraw-compact-pen-option-long-pressing");
      pendingPenLongPress = {
        didDelete: false,
        pen,
        ...
        timer: window.setTimeout(() => {
          ...
          const pending = pendingPenLongPress;
          pending.didDelete = true;
          pending.target.removeClass(
            "excalidraw-compact-pen-option-long-pressing",
          );
          pendingPenLongPress = null;
          this.removeCompactToolbarPenPreset(pending.pen);
        }, 650),
      };
    };
    ...
    const onPointerDown = (event: PointerEvent) => {
      ...
      const addPenButton = target?.closest<HTMLElement>(
        ".excalidraw-compact-pen-add",
      );
      if (addPenButton) {
        event.preventDefault();
        event.stopPropagation();
        event.stopImmediatePropagation();
        this.addCompactToolbarPenPreset();
        return;
      }
      const penOption = target?.closest<HTMLElement>(
        ".excalidraw-compact-pen-option",
      );
      if (penOption) {
        event.preventDefault();
        event.stopPropagation();
        event.stopImmediatePropagation();
        const pen = penOption.dataset.pen;
        if (this.isCompactToolbarPenId(pen)) {
          this.applyCompactToolbarPen(pen);
          startPendingPenLongPress(penOption, event, pen);
        }
        return;
      }
      if (toolHasTestId(target, "toolbar-line")) {
        this.contentEl.removeClass("excalidraw-compact-pen-menu-visible");
        toggleSubmenu("shape");
        return;
      }
      if (toolHasTestId(target, "toolbar-freedraw")) {
        event.preventDefault();
        event.stopPropagation();
        event.stopImmediatePropagation();
        this.ensureCompactPenMenu();
        this.applyCompactToolbarPen(this.compactToolbarActivePen);
        return;
      }
      if (isShapeSubmenuTarget(target)) {
        this.contentEl.removeClass("excalidraw-compact-pen-menu-visible");
        setSubmenu("shape");
        return;
      }
      if (target?.closest(".reset-zoom-button")) {
        this.contentEl.removeClass("excalidraw-compact-pen-menu-visible");
        toggleSubmenu("zoom");
        return;
      }
      if (target?.closest(".zoom-out-button, .zoom-in-button")) {
        this.contentEl.removeClass("excalidraw-compact-pen-menu-visible");
        setSubmenu("zoom");
        return;
      }
      if (target?.closest(".App-toolbar__extra-tools-trigger")) {
        this.contentEl.removeClass("excalidraw-compact-pen-menu-visible");
        toggleSubmenu("more");
        return;
      }
      if (target?.closest(".ToolIcon__lock")) {
        this.contentEl.removeClass("excalidraw-compact-pen-menu-visible");
        setSubmenu("more");
        return;
      }
      if (
        isZoomSubmenuTarget(target) ||
        isPenSubmenuTarget(target) ||
        isMoreSubmenuTarget(target)
      ) {
        return;
      }
      this.contentEl.removeClass("excalidraw-compact-pen-menu-visible");
    ...
    const onPointerMove = (event: PointerEvent) => {
      if (
        !pendingPenLongPress ||
        pendingPenLongPress.pointerId !== event.pointerId
      ) {
        return;
      }
      if (
        Math.hypot(
          event.clientX - pendingPenLongPress.startX,
          event.clientY - pendingPenLongPress.startY,
        ) > 7
      ) {
        clearPendingPenLongPress();
      }
    ...
    this.registerDomEvent(this.contentEl, "pointerup", onPointerEnd, {
      capture: true,
    });
    this.registerDomEvent(this.contentEl, "pointercancel", onPointerEnd, {
      capture: true,
    });
    this.updateCompactToolbarLayout();
    this.updateCompactSelectedElementActionsPosition();
  ...
  private updateCompactToolbarLayout() {
    ...
    try {
      ...
      const topBarRect = topBar?.getBoundingClientRect() ?? toolbarRect;
      const bottomBarRect = bottomBar.getBoundingClientRect();
      const zoomRect = zoomActions.getBoundingClientRect();
      ...
      const visibleMiscRects = miscTools
        ? Array.from(miscTools.children)
            .map((child) => ({
              ...
              style: this.ownerWindow.getComputedStyle(child),
            }))
            .filter(
              ({ rect, style }) =>
                rect.width > 0 &&
                rect.height > 0 &&
                rect.top < topBarRect.bottom + 4 &&
                rect.bottom > topBarRect.top - 4 &&
                style.display !== "none" &&
                style.visibility !== "hidden" &&
                Number(style.opacity || "1") > 0.01,
            )
            .map(({ rect }) => rect)
        : [];
      const toolbarContentRight = Math.max(
        toolbarRect.right,
        ...visibleMiscRects.map((rect) => rect.right),
      );
      const topBarLeft = topBarRect.left || toolbarRect.left;
      ...
      if (penToolRect) {
        ...
        const penMenuWidth = this.getCompactToolbarPenMenuWidth();
        const penMenuHeight = 22;
        const preferredPenMenuLeft =
          penToolRect.left + penToolRect.width / 2 - penMenuWidth / 2;
        const preferredPenMenuTop = penToolRect.bottom + 5;
        const penMenuLeft = Math.min(
          Math.max(wrapperRect.left + 8, preferredPenMenuLeft),
          Math.max(wrapperRect.left + 8, wrapperRect.right - penMenuWidth - 8),
        );
        const penMenuTop = Math.min(
          Math.max(wrapperRect.top + 8, preferredPenMenuTop),
          Math.max(wrapperRect.top + 8, wrapperRect.bottom - penMenuHeight - 8),
        );
        this.contentEl.style.setProperty(
          "--excalidraw-compact-pen-menu-left",
          `${Math.ceil(penMenuLeft - fixedOriginRect.left)}px`,
      ...
      const zoomLeft = Math.max(
        56,
        Math.ceil(toolbarContentRight - bottomBarLeft + 2),
      );
      const clampedZoomLeft = Math.min(
        Math.max(0, maxToolbarWidth - zoomWidth),
        zoomLeft,
      );
      const toolbarWidth = Math.min(
        maxToolbarWidth,
        Math.max(92, clampedZoomLeft + zoomWidth + 8),
      );
      ...
      const settingsTop = Math.ceil(bottomBarRect.bottom + 8);
      this.contentEl.style.setProperty(
        "--excalidraw-compact-settings-left",
        `${Math.ceil(Math.max(wrapperRect.left + 8, 8))}px`,
      );
      this.contentEl.style.setProperty(
      ...
      this.contentEl.style.setProperty(
        ...
        `${Math.max(120, Math.floor(wrapperRect.bottom - settingsTop - 8))}px`,
      );
      this.contentEl.style.setProperty(
        "--excalidraw-compact-settings-max-width",
        `${Math.max(172, Math.floor(wrapperRect.width - 16))}px`,
      );
    } catch {
  ...
  private updateCompactSelectedElementActionsPosition() {
    ...
    try {
      ...
      if (selectedElements.length === 0) {
        ...
        const activeCompactPenButton =
          activeToolType === "freedraw"
            ? this.contentEl.querySelector<HTMLElement>(
                `.excalidraw-compact-pen-option-${this.compactToolbarActivePen}`,
              )
            : null;
        const activeCompactPenRect =
          activeCompactPenButton?.getBoundingClientRect();
        ...
        const visibleShapeSubmenuBottom =
          this.compactToolbarOpenSubmenu === "shape"
            ? Math.max(
                0,
                ...Array.from(
                  this.queryCompactToolbarElements<HTMLElement>(
                  ...
                  .filter((tool): tool is HTMLElement => Boolean(tool))
                  .filter((tool) => {
                    const style = this.ownerWindow.getComputedStyle(tool);
                    return (
                      style.display !== "none" &&
                  ...
                  .map((tool) => tool.getBoundingClientRect().bottom),
              )
            : 0;
        const left =
          clamp(
            activeToolRect.left + activeToolRect.width / 2 - actionsWidth / 2,
            minLeft,
            maxLeft,
          ) - fixedOriginRect.left;
        const top =
          clamp(
            Math.max(activeToolRect.bottom + 6, visibleShapeSubmenuBottom + 6),
            minTop,
            maxTop,
          ) - fixedOriginRect.top;

        this.contentEl.style.setProperty(
          "--excalidraw-selected-actions-left",
          `${left}px`,
        );
        this.contentEl.style.setProperty(
          "--excalidraw-selected-actions-top",
          `${top}px`,
        );
        this.contentEl.addClass("excalidraw-compact-floating-actions");
        this.contentEl.removeClass("excalidraw-compact-selected-actions");
        return;
      }
      ...
      const preferredTop =
        aboveTop >= minTop
          ? aboveTop
          : belowTop + actionsHeight <= maxTop
            ? belowTop
            : visibleBottom + selectedActionGap;
      const top = clamp(
        preferredTop,
        minTop,
        maxTop,
      ...
      this.contentEl.addClass("excalidraw-compact-floating-actions");
      this.contentEl.addClass("excalidraw-compact-selected-actions");
    } catch (error) {
      this.contentEl.removeClass("excalidraw-compact-floating-actions");
      this.contentEl.removeClass("excalidraw-compact-selected-actions");
    }
