...
function getMarkdownInteractiveEmbedPreviewSpec(
  ...
  const imageSource = linkParts.ref
    ? `${resolvedFile.path}#${linkParts.isBlockRef ? "^" : ""}${linkParts.ref}`
    : resolvedFile;
...
export async function insertEmbeddableToView(
  ...
  if (markdownPreviewSpec) {
    const embeddableStrokeColor = ea.style.strokeColor;
    const embeddableBackgroundColor = ea.style.backgroundColor;
    ea.style.strokeColor = "transparent";
    ea.style.backgroundColor = "transparent";
    try {
      ...
      ea.style.strokeColor = embeddableStrokeColor;
      ea.style.backgroundColor = embeddableBackgroundColor;
    }
    if (previewImageId) {
